...
def extract_all_groups_local(target_col='label'):
    ...
    """
    Feature Extraction based on Random Forest

    Excludes label and raw time series, computes for every group the importance 
    and returns also weighted average and std variance amongst groups, to highlight
    which features would perform well individually but yield worse federated outputs
    """
    ...
    for i in range(9):
        ...
        # Clean data: Drop non-numeric columns and target
    ...
    # Visualizing the Top 20 Features
